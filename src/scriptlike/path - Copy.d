...
///ditto
int runShell(Range)(Range commandParts) 
	if(isInputRange!Range && is(ElementType!Range == string))
{
	return runShell(generateCommand(commandParts));
}

///ditto
int runShell(C, Range)(PathT!C workingDirectory, Range commandParts) 
	if(isInputRange!Range && is(ElementType!Range == string))
{
	return runShell(workingDirectory, generateCommand(commandParts));
}

/// Generates a command string from an input range of arguments.
string generateCommand(Range)(Range commandParts) 
	if(isInputRange!Range && is(ElementType!Range == string))
{
	Appender!string cmdBuf;
	while(!commandParts.empty)
	{
		cmdBuf ~= escapeShellPath(commandParts.front);
		commandParts.popFront();
...
unittest
{
	...
	writeln("Running 'scriptlike.d' unittests: generateCommand");

	version(Windows)
		assert(generateCommand([`path/foo bar.abc`, `-a`, `Hello world`, `--stuff`]) == `"path/foo bar.abc" -a "Hello world" --stuff`);
	else version(Posix)
		assert(generateCommand([`path/foo bar.abc`, `-a`, `Hello world`, `--stuff`]) == `'path/foo bar.abc' -a 'Hello world' --stuff`);
